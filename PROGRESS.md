...
_Обновлено: 2026-07-21. Последний коммит на момент аудита: `0598e76`._
Связанные доки: `PLAN.md`, `CONTENT_STRATEGY.md`, `VALUE_MAP.md`.
Память ассистента: `MEMORY.md` (раздел `bezb-content-vs-process` — подробный лог).

---

## 0. Решение о перезапуске — 2026-07-21

Текущая версия технически богата, но продукт и контент перестали быть полезными
самому автору: он больше не хочет их читать и пересылать. Поэтому прежний план
наращивания функций и ежедневного контент-конвейера остановлен.

Новое ядро: **реальный публичный портфель, регулярное пополнение, каждое решение с
исходной причиной и последующая проверка без переписывания истории**. Подробное
решение — в `PLAN.md`, редакционная система — в `CONTENT_STRATEGY.md`.

Зафиксировано при аудите продовой публичной части:

- портфель уже не пуст: 2026-07-21 API показывал около 32,6 тыс. ₽ / $416,7,
  доходность около +9,1% в ₽ и +5,4% в $, а также около $100 в USDT;
- в ленте Mini App было 34 опубликованных материала, 14 разных типов, медианная
  длина 1689 знаков;
- внутри Mini App зафиксировано 7 реакций на 5 материала и 0 комментариев. Это не
  заменяет Telegram-аналитику, которую ещё нужно снять отдельно;
- код AI разрешает писать от первого лица и генерировать «личные» истории. Это
  признано несовместимым с обещанием прозрачности.

До завершения четырёхнедельного теста не расширяем игры, премиум, AI-функции и
контентные каналы. Следующая реализация: остановить расписанный конвейер, запретить
AI выдумывать голос автора и упростить главный экран вокруг эксперимента.

Дополнение 2026-07-21: из серверной очереди безвозвратно удалены 53 старых
неопубликованных AI-черновика. Content Studio в локальном коде заменена на
«Редактор публикации»: он не хранит очередь, не генерирует текст или картинки и не
обращается к AI. Разрешены готовый текст, предпросмотр, опциональное собственное
изображение и прямая публикация с кнопкой портфеля. Старые API очереди, генерации
рубрик и Дзена удалены. Изменения задеплоены 2026-07-21 коммитом `a33d288`:
`bezb-api` и `bezb-bot` активны, новая Mini App опубликована через GitHub Pages,
продовые проверки старых и новых маршрутов пройдены.

Разделы ниже сохраняют техническую инвентаризацию уже созданной системы. Их старые
приоритеты и действия владельца не являются текущим планом; текущий порядок работ
зафиксирован только в `PLAN.md`.
...
## 6. Прежний бэклог — справочно, не текущий приоритет
...
## 7. Прежние действия владельца — отложено
...
## 8. Тех-долг / заметки
- Публичный портфель Без Б на сервере уже заполнен; прежняя пометка о пустом
  портфеле устарела. Актуальные цифры всегда брать из продового API.
