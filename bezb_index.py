...
def _label(v: int) -> str:
    # «Режим рынка»: 0 = перепроданность/капитуляция, 100 = перегрев/эйфория.
    # Не Fear&Greed (тот — отдельная карточка); это композит режима и перегрева.
    return {"extreme_fear": "Капитуляция", "fear": "Охлаждение", "neutral": "Баланс",
            "greed": "Разгон", "extreme_greed": "Перегрев"}[_zone(v)]
...
def context() -> str:
    ...
    return (f"Индекс Без Б (режим рынка) = {d['value']}/100 — {d['label']} "
            f"(0 = перепроданность/капитуляция, 100 = перегрев/эйфория). Главные факторы: {top}.")
